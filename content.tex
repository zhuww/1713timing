...
modeled, with residuals of weighted root mean square of $\sim 118$~ns, by the
spin down and proper motion of the pulsar, 
a comprehensive binary model that included the 3-D orientation 
of the orbit in the sky, and a noise model that incorporates potential
correlated noises such as jitter and red noies. 
...
[1.34(9)~\Msun] and white dwarf [0.291(11)~\Msun] and the parallax
 [$\pi=$0.87(3)~mas].  
...
find that the intrinsic change in orbital period is $\dot{P}^{\rm Int}_{\rm
...
%\bfref{
%Through pulse timing analysis, we detect evidences of both pulse
%jitter noises and red timing noises in our data.
%}
\end{abstract}
...
%WRMS of $\sim 118$ns (Figure \ref{fig:detres}, Table \ref{tab:wrms}).
...
companion's mass $M_{\rm c} = 0.291\pm0.011$~\Msun and the pulsar $M_{\rm
PSR}=1.34\pm0.1=09$~\Msun\, are in good agreement with the previously measured values \cite{sns+05}.
...
spectral index is inconsistent with the $\gamma_{\rm red} = -2.92^{+0.44}_{-0.09}$
we measured from our red noise model.
...
the estimated maximum likelihood red noise spectral index 
 suggests that it could be due to
...
0.48\pm0.18\times10^{-12}$s~s$^{-1}$ (Table \ref{tab:par1} and \ref{tab:par2}).
...
$0.3\pm1.1\times10^{-12}$~yr$^{-1}$ ($<0.029H_0$ based on 3-$\sigma$ limit), where $H_0$ is the Hubble constant. 
...
%Timing residuals of MSPs are believed to contain noise from timing
...
%time-accumulative noise. This indicates that, unlike other younger pulsars,
%PSR~J1713+0747's timing residual consists of mostly white noise. Understanding the nature 
%of the noise in these MSPs is important for making predictions of detecting
