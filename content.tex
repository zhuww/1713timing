...
binary orbit's apparent decay due to motion 
...
Millisecond pulsars (MSPs) are very stable rotators due to their enormous angular momentum. PSR~J1713+0747 is one of the most stable MSPs, and it resides in a wide binary orbit with a white dwarf companion (Section \ref{sec:model}). 
...
The system's distance is well-measured through a timing parallax. We also detect
...
We also measure an apparent decay of the binary orbit due to the system's
...
We employ the pulsar timing tool \textit{TEMPO}
...
\footnote{{\bf We used a Metropolis algorithm assuming the goodness of fit of
the timing model is proportional to $e^{-\chi^2/2}$. The procedure is similar
to that in the {\textbf TEMPO2} MCMC plugin \citep{dpr+10}. The code for running this
...
companion's mass $M_{\rm c} = 0.283\pm0.004$ and the pulsar $M_{\rm
PSR}=1.29\pm0.03$ are in good agreement with the previously measured values \cite{sns+05}.
...
value of DM value for each group.  The same procedure was carried out in
...
follow the same characterization. It went trough a steep drop and recover 
around 2009 for few hundreds days, Conversely, the overall long-term variation is smaller
compared with the 2009 event. As a result, its structure function (Figure \ref{fig:dmx}) follows a
flatter spectrum than the Kolmogorov one.
...
Timing noise is often significant in younger pulsars, and often
...
{\bf These mechanisms could lead to accumulative random perturbations in the pulsar's phase of
pulse arrival, spin rate, or spin-down rate. 
...
denote one of the $N$ TOAs from the respective observation epoch.
...
As a result we expect the residuals' WRMS to be slightly
...
Please note that these ``corrected RMSs'' serve to demonstrate the effect of ``overfitting''.
...
noise may contain some red and non-Gaussian noises. A more realistic noise
analysis is needed for the 21-yr data set along the lines of Dolch et~al
(2014, in preparation).
%More thorough and
...
$\sim20$~min NANOGrav observation, one expect $\sim 50$~ns of jitter noise. 
...
$4\pm8\times10^{-13}$~yr$^{-1}$ ($<0.0123H_0$ based on 3-$\sigma$ limit), where $H_0$ is the Hubble constant. 
...
be partly caused by the event of large DM variation happened in 2009. This
2009 event might have increased the average DM variation in timescale of
