...
series spanning 14~years within which no unknown time offset has to be fitted. 
Unfortunately, there is no
over-lapping observations connecting these 14~yr data with the earliest  Mark~III TOAs. 
...
independent from one TOA to another, whereas the correlated noises are not. 
For instance, the template matching errors and the radiometer noises are 
uncorrelated noises. In contrast, the pulse jitter noise affects all the TOAs 
in the same epoch simultaneously, therefore, it is correlated in every epoch.
...
\begin{equation}
\label{eq:bigC}
p(\msR|\vec \phi) = \frac{1}{\sqrt{\det(2 \pi \msC)}}
...
^{\rm pre} - M \epsilon$, here $\delta t^{\rm pre}$ is the pre-fit residual,
$M$ is the design matrix of the timing model and $\epsilon$ denote small
offsets in the timing parameters. 

The general noise covariance matrix $\msC$ could be used to describe both
...
noises would give rise to non-zero non-diagonal elements. If $\msC_{ij}(i\neq
j)\neq 0$ indicates that $\msR_{i}$ is correlated with $\msR_{j}$. In the case
of pulse jitter noise, only the TOAs from the same epoch would be correlated
with each other, thus $\msC$ would be a block diagonal matrix $\msS$. If both types
of noise present, then $\msC = \msD + \msS$.
...
parameter. Timing software then enlarge the template matching errors $\sigma$ of 
TOAs in the same group by the value of EFAC,
and EQUAD to $\sigma$ in quadrature.
We continue to use the EFAC $E$ and EQUAD $Q$ parameterization for modeling extra
...
In order to model the correlated jitter noise, we employ another noise
parameter ECORR. The value of ECORR parameter $J$ parameterize the correlated
noise covariance matrix $\msS_{ij} = J_k^2$, where TOA $i$ and $j$ come from
the same sub-integration, and $k$ is one of the backend-receiver group.
}
