...
The system's distance is well-measured via timing parallax, and we detect the changes in the length of the projected semi-major axis, through which we can infer the orientation of the orbit in the sky.
...
one such as the change of the gravitational constant $G$. 
The interesting conjecture of $G$ varying in a cosmological timescale was first 
...
the scalar field will also expand, thus lead to a changing local gravitational constant. 
...
$D_{\phi}(\tau)=(2\pi C/f^2)\left( [DM(t+\tau)-DM(t)]^2\right)$, where $\tau$ 
