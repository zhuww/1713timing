...
modeled, with residuals of weighted root mean square of $\sim 92$~ns, by a
...
long-timescale correlated noises such as jitter and red noises. 
...
The intrinsic change in orbital period is $\dot{P}^{\rm Int}_{\rm
b}=-0.20\pm0.17\times10^{-12}$s~s$^{-1}$, not distinguishable from zero.
This result combined with the measured $\dot{P}^{\rm Int}_{\rm b}$ of other pulsars 
can place limits on potential changes in the gravitational constant $\dot{G}$
as predicted in some alternative theories of gravitation. 
$\dot{G}/G$ is found to be consistent with zero [$-0.6(1.1)\times10^{-12}$~yr$^{-1}$] and at 
least a factor of $31$ (99.7\% confidence level) slower than the average expansion 
...
angular momentum. PSR~J1713+0747 is a MSP residing in a wide binary orbit with a white dwarf companion (Section \ref{sec:model}). 
...
over-lapping observation connecting these 16~yr data with the earliest  Mark~III ToAs. 
...
we employed a general noise model that parameterizes both uncorrelated noise and
...
\citet{sns+05} also successfully describe the 21-yr data set, except for the
...
characterizes
epoch, time, and frequency independent random noise left in the data. %is generally the dominant source of noise in pulsar timing data.
...
%WRMS of $\sim 92$ns (Figure \ref{fig:detres}, Table \ref{tab:wrms}).
...
The same feature was also observed by \citet{sns+05}, and they modeled it
...
DM variation can affect the timing of high-precision pulsar significantly.
...
This averaging was done using a previous timing solution in previous work
...
It has been shown that DM variation of some pulsars are consistent with
...
(Figure \ref{fig:dmx}). Conversely, it went trough a steep drop and recovery 
around 2008. If such fast DM changes are the result of variations in the ISM along
light of sight, such ISM variation does not fit the general characteristics of Kolmogorov median. 
...
from \citet{sc10}.  
%While the integrated RMS of the red noise is consistent with the prediction of
%\cite{sc10}, the red noise spectral index is not. \cite{sc10} predict that the
%spectral index of the red noise will be $\sim-5$, whereas the spectral index
%that we measure is $\gamma_{\rm red} = -2.17^{+0.44}_{-0.09}$. 
...
b})=1.0\pm2.3\times10^{-11}$~yr$^{-1}$ using the timing of binary PSR~1913+16. 
...
\citet{fle+14} showed that $\dot{G}$ can be constrained to 
...
slightly improving the single pulsar limits from earlier data of this pulsar
...
Meanwhile, the precise measurements of the PSR J1713+0747's orbital eccentricity and
...
the scale law extrapolated from large sample study of timing noise in \citet{hlk10}
...
This is consistent with the non-detection of red noise in the 5-yr
