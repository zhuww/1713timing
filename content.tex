...
pulsar---J1713+0747. We improve measurements many of the pulsar's parameters.
Most interestingly, we detect the its orbital decay due to Galactic
...
precisely 
the pulsar's distance, proper motion, its binary system's masses, orbit, 
...
binaries. The Luna Laser Ranging (LLR) experiment measured the
...
array project. It is observed monthly in L band and S band by the Arecibo
...
of the later NANOGrav observations. The timing observations were performed
using multiple generations of pulsar instruments. \citet{sns+05} observed
J1713+0747 using the Princeton Mark III \citep{skn+92}, Mark IV
\citep{sst+00}, and the Arecibo Berkeley Pulsar Processor (ABPP) system. 
In the early years of NANOGrav observations, pulsar data are collected with the Astronomical Signal
Processor (ASP) and its Green Bank counterpart GASP \citep{dem07}. We started using
...
smaller radio meter noise. Indeed some improvement in timing residual was
observed when upgrading from ASP to PUPPI, but less than predicted by the
radiometer equation (see Section \ref{sec:nosie} for details). Perhaps a
...
$\sim30$~min NANOGrav observation, one expect $\sim 40$~ns of jitter noise. 
...
the best constrain of this type ($\dot{G}/G=0.7\pm7.6\times10^{-13}$~yr$^{-1}$) from the solar system LLR experiment \citep{hmb10}, the $\dot{G}$ limit from pulsar timing is still an valuable independent test.  
...
This makes J1713+0747 one of the most well timed pulsars.

%Noise budget and gravitational wave limit
The `rescaled' WRMS of the post-fit residuals (Table \ref{tab:wrms}) provide a crude estimation of
