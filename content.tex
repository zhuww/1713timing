...
frame effect (PFE), because the violation of SEP or the existence of PFE could polarize the
...
constraint on the violation of SEP and the significance of PFE from our 21 yrs of observation of PSR~J1713+0747.
...
two instruments, and found that ABPP 1410-MHz ToAs trail those of Mark IV
by $\simeq 0.46791$ (xxx please change this to the precise number
plus/minus the uncertainty) in pulse phase. In the timing modeling, we fix
...
\mu_\delta$) on the sky and its parallax $\pi$ are also measured through timing modeling. 
The distance inferred from our parallax
measurement is $D_{\rm PSR}=$1.15(3)~kpc. This distance is consistent with and
...
The timing parameters and uncertainties are marginalized using a generalized least
square (GLS) approach available in the {\sc TEMPO} software package. 
...
Other sources of noise must be modeled separately. The solution to this problem has been discussed extensively in \citet{vl13, ell13, vv14a, vv14, abb+14} and J. A. Ellis (PhD thesis 2014). In this section, we follow the noise model used in \citet{abb+14} closely but will make a few modifications.
...
characterizes noise that is completely correlated for all TOAs in a given
...
last term $\mathbf{n}$ describes a Gaussian white noise process that
...
corrected TOA uncertainties) in logrithm scale in blue curve, along with a
zero-mean unit-variance Gaussian distribution marked as red dashed line.}
\end{figure*} 
...
likelihood timing model parameters via the equivalent of a 
GLS fit and the maximum likelihood red noise realization present in the data. 
...
The posterior distribution of the noise parameters are sampled using a Markov Chain Monte
...
$\mu$s/${\rm yr}^{-1/2}$, and $\gamma_{\rm red}\in[0, 7]$. 
...
of the red noise. This noise model describes the data well as we can see in Figure \ref{fig:detres} in which the maximum likelihood realizations of red and jitter noise are subtracted out. We see from the figure that both the high and low frequency residuals (with red and jitter noise realizations subtracted) are white (described by our EFAC and EQUAD parameters) and the weighted residuals follow a zero-mean unit-variance Gaussian distribution. We do note that the normalized residuals do not follow a guassian distribution outside of the 3-sigma range; however, since the full data set has 14,750 TOAs, this means that only $\sim$ 50 do not follow a gaussian distribution and will not significantly affect the results presented here. 
...
\citet{lwj+09} and \citet{fwe+12} showed that a generic test of $\dot{G}/G$ can be archived using pulsar
...
\mu}$s \citep{lk05}, $\mu$ is the reduced mass ($m_pm_c/M$) of the system,
$\kappa_D$ is a dipole
...
$\hat{\alpha}_3$ is the strong-field version of the PPN
parameters $\alpha_3$ that characterizes the significance of PFE.
