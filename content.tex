...
However, the DM fitting process, which minimize the residuals, have a tendency to ``overfit'' or 
...
J1713+0747, $\dot{P}_{\rm b} = 0.40(15)\times10^{-15}$s~s$^{-1}$. 
...
pulsar's rotational energy $\dot{M_{\rm PSR}}<\dot{E}/c^2$, which is
...
all three parameters of PSR J1713+0747 to very high precision, which enable us to 
...
thanks to PSR J1713+0747's exceptionally large $P_{\rm b}$, and can be used to put new limit on the alternative theories of Gravity.

Some alternative theories of gravitation predict a dipole gravitational
radiation from compact binaries (\citealt{Will93, Will01, lwj+09, fwe+12} and reference therein). Such effect arise when the two bodies of binary are very different in terms of their self gravity, or in another word, their compactness.
...
theories that violates the SEP.
...
This constrain on $\dot{G}$ is more than a fact of two more strident than previous pulsar-based constrain \citep{fwe+12}. 
...
this new pulsar-timing  $\dot{G}$ limit is a strident and independent test.  
...
white dwarf, the J1713+0747 pulsar binary is an excellent laboratory for testing 
...
After correcting for ``over-fitting'', the ``rescaled'' weighted RMS of the post-fit residuals (Table \ref{tab:wrms}) provide a crude estimation of
...
In another word, the change rate of gravitational constant has to be a factor of $>50$ slower than the Universe's expansion rate.
...
discovered pulsar triple system PSR J0337+1715 \citep{rsa+14}. In this case, the
...
the free fall of objects of different self-gravity.
