...
least a factor of $\ll50$ slower than the expansion rate of the Universe. Such a
...
resolution. It is not possible to precisely measure DM from MarkII TOAs.
...
$\sim10^{-4}$. 
This DM variation may lead to red-noise timing residuals as discussed in \citealt{kcs+13}.
...
also assumed the mid-frequency of the frequency band being the center frequency 
of the TOAs, which does not properly account for pulse profile evolution in
frequency, and therefore, are likely to produce TOAs that are slightly offset. We think this may be why the FD and
...
\label{sec:noise}
The pulsar timing residuals (Figure \ref{fig:res}) could contain important
information, like the signal of gravitational waves (GW). However, the current
residuals of PSR J1713+0747 is likely dominated by radiometer noises, DM
variations and other sources of noises. This section discuss J1713+0747's
timing noises, i.e. pulse arrival time variations due to spin irregularities.
...
\end{equation}
Here $t=10^8$~s$\sim 3$~yr is a fiducial time scale close to the average observation time span in \citet{ant94}.
...
Instead, we used a polynomial of the logarithm of
frequency ({\it FD} model by P. Demorest et~al in prep.; see Figure \ref{fig:FD}) to fit for and removed the profile-frequency evolution. This model can successfully remove the extra frequency-dependent residuals and it only uses four parameters.
...
However, the DM fitting process have a tendency to ``overfit'' or 
``absorb'' part of the signals or noises in the residual, making the residuals look more flat than they actually are. 
This is especially significant for the lower band residuals, because
the DM values affects the lower frequency TOAs significantly more than the
higher frequency TOAs. 
As a result we expect the weighted WRMS measured from the
averaged residuals be under-estimated. We can crudely quantify
this effect by injecting random Gaussian noise into the timing TOAs and
measure the responds of the post-fit residuals. Figure \ref{fig:overfit} show
...
and the inferred/corrected RMS based on noise injection simulations. One caveat 
of this approach is that the injected noise are white, while the actual timing
residuals may compose of red noises. 


The great advances in instrumentation has significantly brought down the 
level timing residuals, making our timing model more and more accurate.
...
This jitter noise could be a significant part of the observed residuals.
